...
function flettSpiller(fam, p) {
  ...
  if (!navn || FARLIGE.has(navn)) return;
  if (!fam.players[navn] && Object.keys(fam.players).length >= MAKS_SPILLERE) return;

  const cur = fam.players[navn] || { name: navn, kr: 0, caught: {}, ts: 0 };
  const inn = p.caught && typeof p.caught === "object" ? p.caught : {};

  for (const art of Object.keys(inn).slice(0, MAKS_ARTER)) {
    if (FARLIGE.has(art) || typeof inn[art] !== "object" || !inn[art]) continue;
    const a = cur.caught[art] || { count: 0, bestLen: 0, bestWt: 0 };
    const b = inn[art];
    const flettet = {
      count: Math.max(a.count || 0, tall(b.count, 1e6)),
      bestLen: Math.max(a.bestLen || 0, tall(b.bestLen, 500)),
      bestWt: Math.max(a.bestWt || 0, tall(b.bestWt, 600)),
    };
  ...
  const ts = tall(p.ts, 4102444800000); // aldri lenger frem enn år 2100
...
export default async (req) => {
  ...
  if (body.player) flettSpiller(fam, body.player);
